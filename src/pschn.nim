...
proc getAuth(): string =
  loadEnv()
  let pub = getEnv("PSCHN_SENDCLOUD_PUBLIC")
  let sec = getEnv("PSCHN_SENDCLOUD_PRIVATE")
  if pub == "" or sec == "":
    quit "Error: PSCHN_SENDCLOUD_PUBLIC and PSCHN_SENDCLOUD_PRIVATE must be set", 1
  encode(pub & ":" & sec)
